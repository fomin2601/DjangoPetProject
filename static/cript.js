...
function encrypt(message, name, room) {
    //let pubKeyFilename = local('%s_pubkey.txt' %name)
    let namePublicKey = String(`${username}_${room}_pubkey`)
    let namePrivateKey = String(`${username}_${room}_privkey`)
    let publicKey = JSON.parse(localStorage.getItem(namePublicKey))
    let privateKey = JSON.parse(localStorage.getItem(namePrivateKey))
